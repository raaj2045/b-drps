/*
 * Section 6: latency decomposition under a mainnet-like composite delay model.
 *
 ...
 * testnet latency is made. The separately measured Section 2 (real 12s
 * interval mining) provides the empirical cross-check for the inclusion term.
 *
 * Run standalone:  npx hardhat run scripts/benchmark/latency-v2.js
...
function renderSection(data) {
  const lines = ["## 6. Latency decomposition (composite mainnet-sim model)\n"];
  lines.push(
  ...
  lines.push(
    ...
    "Section 2 (real 12 s interval mining) independently cross-checks the " +
    "inclusion term.\n"
  );
  ...
  lines.push("![Latency stability across N](./figures/latency_v2_by_n.png)");
  lines.push("Raw data: [latency_v2.csv](./latency_v2.csv)");
  return lines.join("\n");
}
...
async function main() {
  console.log("[latency-v2] running...");
  const data = await run();
  writeCsvFile(data);
  writeSection("06-latency-decomposition", renderSection(data));
  console.log("[latency-v2] done");
}
...
module.exports = { run, renderSection, writeCsvFile, LATENCY_V2_NS };
